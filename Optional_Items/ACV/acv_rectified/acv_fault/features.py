...
# `mode_invalid_rate` (substring "invalid" match against the free-text
# "ACV Running Mode" field) was dropped after an MIT-ML/rail-technician
# multicollinearity review: it correlates 0.9998 with `info_invalid_rate`
# (a direct "ACV Information Valid" flag) across all 48 training car-rows -
# VIF ~3,290, i.e. almost entirely redundant with info_invalid_rate rather
# than an independent signal. `info_invalid_rate` is kept as the sole
# validity feature since it reads a direct status flag rather than a
# substring match against free text, making it more robust across the
# "standard" vs "rich" telemetry schema variants. `cool_gap_mean`/`_p90`/
# `_max` were checked too (VIF 5.47/4.05/3.38) and left as-is: below the
# textbook VIF>10 concern threshold, and each genuinely adds distinguishable
# center-vs-tail information about the same underlying gap distribution.
